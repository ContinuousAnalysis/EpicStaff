...
class ElevenLabsClientEventHandler:
    ...
    Includes real-time 24kHz to 16kHz audio resampling.
    """
    ...
    async def _handle_audio_append(self, data: Dict[str, Any]) -> None:
        ...
        try:
            # Декодирование входящего аудио (OpenAI frontend: PCM 16-bit Mono, 24kHz)
            pcm_data_24k = base64.b64decode(audio_b64)
            audio_array_24k = np.frombuffer(pcm_data_24k, dtype=np.int16)

            # Быстрый ресемплинг из 24kHz в 16kHz для ElevenLabs (коэффициент 2/3)
            # В реальном времени децимация массивов работает быстрее тяжелых фильтров
            indices = np.arange(0, len(audio_array_24k), 1.5).astype(np.int32)
            audio_array_16k = audio_array_24k[indices]

            # Кодирование обратно в Base64 для отправки
            resampled_b64 = base64.b64encode(audio_array_16k.tobytes()).decode("utf-8")

            # Правильный формат отправки чанка аудио в ElevenLabs
    ...
    async def _handle_conversation_item_create(self, data: Dict[str, Any]) -> None:
        # ElevenLabs Conversational AI WebSockets не поддерживают прямую отправку текста.
        # Событие игнорируется, чтобы избежать разрыва соединения сервером из-за неверного формата.
        logger.debug(
