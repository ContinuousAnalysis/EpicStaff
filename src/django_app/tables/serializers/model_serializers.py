...
class TaskWriteSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if context_ids is None and "order" in validated_data:
            existing_ids = list(
                TaskContext.objects.filter(task=task).values_list(
                    "context_id", flat=True
                )
            )
            self._update_task_contexts(task, existing_ids)
