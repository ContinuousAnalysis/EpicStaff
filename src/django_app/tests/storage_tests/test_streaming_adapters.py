...
# --- validate_name / validate_stream ---


def test_validate_name_blocks_exe():
    v = FileValidator(limits=_limits())
    with pytest.raises(serializers.ValidationError):
        v.validate_name("evil.exe", 10)


def test_validate_name_blocks_unsupported_archive():
    v = FileValidator(limits=_limits())
    with pytest.raises(serializers.ValidationError):
        v.validate_name("a.rar", 10)


def test_validate_name_blocks_oversize():
    v = FileValidator(limits=_limits(max_file_bytes=100))
    with pytest.raises(serializers.ValidationError):
        v.validate_name("big.bin", 200)


def test_validate_name_ok_and_none_size():
    v = FileValidator(limits=_limits())
    v.validate_name("ok.txt", 10)
    v.validate_name("ok.txt", None)


def test_validate_stream_rejects_exe_in_archive():
    v = FileValidator(limits=_limits())
    archive = _zip({"payload.exe": b"MZ"})
    with pytest.raises(serializers.ValidationError):
        v.validate_stream(archive, "a.zip", 100)
