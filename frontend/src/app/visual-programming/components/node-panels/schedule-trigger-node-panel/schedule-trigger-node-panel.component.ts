...
export class ScheduleTriggerNodePanelComponent extends BaseSidePanel<ScheduleTriggerNodeModel> {
    ...
    private isStrictAutoGeneratedName(name: string): boolean {
        ...
        if (/^Once on \d{2} (?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) \d{4} at \d{2}:\d{2}$/.test(name))
            return true;
    ...
    private generateScheduleName(data: ScheduleTriggerNodeData): string {
        if (data.runMode === 'once') {
            if (data.startDateTime) {
                const timePart = data.startDateTime.split('T')[1]?.slice(0, 5) ?? '';
    ...
    createUpdatedNode(): ScheduleTriggerNodeModel {
        ...
        const resolvedName =
            !userChangedName && this.isStrictAutoGeneratedName(this.initialNodeName)
                ? this.generateScheduleName(data)
                : formName;
